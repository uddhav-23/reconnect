...
const UserDashboard: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <Card variant="primary" className="text-center max-w-md">
          ...
          <p className="font-mono text-gray-600 mb-4">Please log in to access this dashboard.</p>
        </Card>
      </div>
    );
  }
  ...
  const stats = [
    ...
    { label: 'Published Blogs', value: userBlogs.length.toString(), icon: BookOpen, color: 'bg-[#00FF80]' },
    { label: 'Achievements', value: userAchievements.length.toString(), icon: Award, color: 'bg-[#0080FF]' },
    { label: 'Connections', value: ((currentUser as any)?.connections?.length || 0).toString(), icon: UserCheck, color: 'bg-[#FF4444]' },
  ];
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-4xl font-black font-mono uppercase text-black transform -skew-y-1 mb-2">
                ...
                <span className={`${roleColors[user.role] || 'text-[#0080FF]'} block`}>DASHBOARD</span>
              </h1>
              <p className="font-mono text-lg text-gray-600 uppercase">
                Welcome back, {currentUser.name}
              </p>
              {user.role === 'alumni' && (currentUser as any).currentPosition && (
                <p className="font-mono text-sm text-[#FF0080] font-bold">
                  {(currentUser as any).currentPosition} at {(currentUser as any).currentCompany}
                </p>
              )}
              {user.role === 'student' && (currentUser as any).degree && (
                <p className="font-mono text-sm text-[#FF8000] font-bold">
              ...
              <div className="bg-white border-4 border-black p-4 transform -rotate-1">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {user.role === 'student' && (currentUser as any).degree && (
                      <p className="font-mono text-sm text-[#FF8000]">
                        {(currentUser as any).degree} - Year {(currentUser as any).currentYear}
                      </p>
                    )}
                  </div>
              ...
              {(currentUser as any).bio && (
                <div className="bg-[#00FF80] border-2 border-black p-4">
                  <p className="font-mono text-sm text-black">
                    <strong>Bio:</strong> {(currentUser as any).bio}
                  </p>
                </div>
              )}
              ...
              {(currentUser as any).skills && (currentUser as any).skills.length > 0 && (
                <div className="bg-[#0080FF] border-2 border-black p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(currentUser as any).skills.map((skill: string) => (
                      <span 
                        key={skill}
        ...
        <Card variant="primary" className="transform rotate-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {userAchievements.map((achievement) => (
              <div
                key={achievement.id}
                className={`bg-[#00FF80] border-4 border-black p-4 transform ${userAchievements.indexOf(achievement) % 2 === 0 ? 'rotate-1' : '-rotate-1'}`}
              >
