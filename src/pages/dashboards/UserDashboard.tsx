...
const UserDashboard: React.FC = () => {
  ...
  const stats = [
    ...
    { label: 'Published Blogs', value: userBlogs.length.toString(), icon: BookOpen },
    { label: 'Achievements', value: userAchievements.length.toString(), icon: Award },
    { label: 'Connections', value: ((currentUser as any)?.connections?.length || 0).toString(), icon: UserCheck },
  ];
              ...
              {(currentUser as any).bio && (
                <div className="bg-neutral-100 dark:bg-neutral-800 rounded-md border border-[var(--border)] p-4">
                  <p className="text-sm text-[var(--fg)]">
                    <strong className="font-medium">Bio:</strong> {(currentUser as any).bio}
                  </p>
                </div>
              )}
